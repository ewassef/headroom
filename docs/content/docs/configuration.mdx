...
3. nearest repo-local `.headroom/tool_policy.json`
...
- OpenCode enforces the same policy for in-process HTTP and shell interception. `require_approval` currently fails closed there because OpenCode does not expose an interactive approval callback yet.
