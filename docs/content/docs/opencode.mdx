...
| `HEADROOM_TOOL_POLICY_JSON` | Optional JSON policy for pre-exec shell + HTTP enforcement in the native plugin transport (`HEADROOM_OPENCODE_TOOL_POLICY_JSON` remains accepted for backward compatibility) |
...
The native plugin can also enforce a small pre-execution policy layer for outbound HTTP requests and child-process shell launches:
