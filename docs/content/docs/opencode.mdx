...
| `HEADROOM_TOOL_POLICY_JSON` | Optional JSON policy for pre-exec shell + HTTP enforcement in the native plugin transport |
...
You can keep the policy in code, inject it with `HEADROOM_TOOL_POLICY_JSON`, point at a centralized file with `HEADROOM_TOOL_POLICY_PATH`, or commit a repo-local `.headroom/tool_policy.json`.
...
- decision logs: JSON lines on stderr with `event="headroom_tool_policy_decision"`
...
4. **Native plugin path**. `HeadroomPlugin` installs Headroom transport interception, optionally enforces tool policy rules before shell/HTTP execution, and uses `HEADROOM_PROXY_URL` or `http://127.0.0.1:8787` to reach the proxy.
...
Check the resolution order above. `HEADROOM_TOOL_POLICY_JSON` wins over every file-based option. Audited hook and plugin decisions are appended to `~/.headroom/tool_policy_audit.jsonl`, which is useful for confirming which rule matched and which source won.
