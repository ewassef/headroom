...
const conformanceCases = JSON.parse(
  fs.readFileSync(
    new URL("../../../tests/fixtures/tool_policy_conformance.json", import.meta.url),
    "utf8",
  ),
) as ConformanceCase[];
...
describe("Headroom OpenCode transport", () => {
  ...
  it("strips the remote policy token from wrapped child process environments", () => {
    const originalSpawn = childProcess.spawn;
    ...
    try {
      ...
      childProcess.spawn("node", ["agent.js"], {
        env: { PATH: "/bin", HEADROOM_TOOL_POLICY_TOKEN: "custom-private-token" },
      });
